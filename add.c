...
int main(int argc, char *args[])/* params, ct1.txt, ct2.txt, lev1, lev2 (lev1 > lev2)*/
{
	FILE *fp;
        ...
        if(row1 > row2)
                ...
                row = row2;
        fmpz_mat_init(c4, row, 1);
        fmpz_mat_init(c3, row*row, 1);
        fmpz_mat_zero(c3);
       // fmpz_mat_print(levhigh);
       // fmpz_mat_print(levlow);
        fmpz_mat_add(c4, levhigh, levlow);
        
        for(i = 0; i < row; i++ ) {
                fmpz_smod(fmpz_mat_entry(c4,i,0),hp->q);
                fmpz_set(fmpz_mat_entry(c3, i, 0), fmpz_mat_entry(c4, i, 0));
        }
        row1 = 1 + hp->next->n;
        ...
        hcrypt_bgv_refresh(ct, c3, pkb, hp->q, hp->next->q, row*row, row1);
